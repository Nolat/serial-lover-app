...
function useConfigChanger(config) {
  ...
  function change(field) {
    if (defaultOptions[field] === undefined)
      throw Error(`default value for ${field} does not exist`);
    return {
      ...config,
      [field]: toggleValue(config[field], defaultOptions[field]),
    };
  }
      ...
      <div>
        <div className="avatar placeholder">
          <Suspense fallback={<Loader />}>
            <Avatar
              className="text-neutral-content rounded-full w-36"
              key={config.sex}
              {...config}
            />
          </Suspense>
